...
- Grilla de tiles `(ancho, alto)`. Cada tile tiene un `Terrain` (enum): `PLAINS`, `FOREST`, `MOUNTAIN`, `WATER` (intransitable), `BRIDGE_H`/`BRIDGE_V` (puentes sobre el agua, transitables a costo 1; el generador los pone en los ríos en lugar de los vados, perpendiculares al curso).
- El agua se dibuja con **autotiling** (`wom/ui/tiling.py`, puro y testeable): además del tile de agua pura hay 9 variantes según la costa — `water_n/s/e/w` (orilla en un lateral), `water_ne/nw/se/sw` (esquinas) y `water_single` (charco rodeado de tierra). Combinaciones sin asset exacto (canales de 1 tile, tres costas) caen a la variante más parecida; los puentes y el borde del mapa cuentan como agua.
